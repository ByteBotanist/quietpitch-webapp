...
useEffect(() => {
  ...
  const applySettings = (s) => {
    ...
    if (Array.isArray(s.notes)) {
      ...
      setOpenNoteIds(
        notes.length ? new Set([notes[0].id]) : new Set()
      );
    }
  };
...
const regulatoryStatus = firm?.disclosure?.regulatoryStatus || "";

      const isUnregistered =
        regulatoryStatus.toLowerCase().includes("not registered") ||
        regulatoryStatus.toLowerCase().includes("educational");

      const contactName = firm.contactName || firm.firmName || "this advisor";
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <div className="text-sm subtle">
          {(() => {
            const name = firm.contactName?.trim();
            if (!name) return "Market Insights";

            return name.toLowerCase().endsWith("s")
              ? `${name}' Market Insights`
              : `${name}'s Market Insights`;
            })()}
        </div>
      </div>
    </div>
...
<div className="mt-6 card p-4 rounded-lg">
  ...
  {isUnregistered
    ...
    : <>Interested in working with <strong>{contactName}</strong>?</>
  }
</p>

<p className="m-0 opacity-90">
  {isUnregistered
    ? `We’ll send your message directly to ${contactName}.`
    : "We’ll pass your info directly to the advisor."
  }
</p>
  </div>
  ...
  >
    {isUnregistered ? "Contact" : "Contact Advisor"}
  </button>
</div>
                ...
                {firm.website && (
                  <li>
                    <a
                      ...
                      href={normalizeUrl(firm.website)}
                      target="_blank"
