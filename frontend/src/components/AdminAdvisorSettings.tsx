...
export default function AdminAdvisorSettings() {
  ...
  useEffect(() => {
    console.log("🎯 UI VALUE", data.disclosure?.regulatoryStatus);
  }, [data.disclosure?.regulatoryStatus]);
...
useEffect(() => {
  ...
  (async () => {
    ...
    try {
      ...
      const merged = normalize({
        ...
        branding: {
         ...
         contact: {
          ...
          ...(cloud?.contact ?? {}),
          name: cloud?.contact?.name ?? "",   // 🔥 force include
        },
      ...
      setTimeout(() => {
        console.log("🔥 STATE AFTER SET", merged);
      }, 0);
      setChartCsv((merged.positions?.chart ?? []).join(', '));
      setSummaryCsv((merged.positions?.summary ?? []).join(', '));
      setLastSaved(merged);
